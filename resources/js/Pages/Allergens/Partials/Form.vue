...
const form = useForm({
    ...
    category: props.allergen?.category ?? props.categoryOptions[0] ?? '',
})
    ...
    <form :id="formId" class="grid gap-5 sm:grid-cols-2" @submit.prevent="submit">
        ...
        <div>
            <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-muted-foreground">
                {{ $t('allergies.catalog.form.label_name') }} <span class="text-vibrant-coral-500">*</span>
            </label>
            <input
                v-model="form.name"
                type="text"
        ...
        <div>
            <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-muted-foreground">
                {{ $t('allergies.catalog.form.label_category') }} <span class="text-vibrant-coral-500">*</span>
            </label>
            <select
        ...
        <div v-if="showActions" class="flex items-center justify-end gap-3 sm:col-span-2">
            ...
            >
                {{ form.processing ? $t('allergies.catalog.form.submitting') : $t('allergies.catalog.form.submit') }}
            </button>
        </div>
    </form>
</template>
