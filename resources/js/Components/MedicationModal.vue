...
async function runSearch(query) {
    try {
        const response = await fetch(route('medications.search', { search: query }), {
            headers: { Accept: 'application/json' },
        })
        const payload = await response.json()
        results.value = payload.medications ?? []
    } catch {
        results.value = []
    } finally {
...
<template>
    <Dialog :open="open" @update:open="handleOpenUpdate">
        <DialogContent class="sm:max-w-lg">
            <DialogHeader>
                <DialogTitle>
                    {{ step === 'search' ? $t('medications.search.heading') : $t('medications.form.heading') }}
                </DialogTitle>
                <DialogDescription>
                    {{ step === 'search' ? $t('medications.search.hint') : $t('medications.form.hint') }}
                </DialogDescription>
            </DialogHeader>
            ...
            <div v-if="step === 'search'" class="grid gap-4">
                ...
                <ul v-else class="max-h-72 divide-y divide-border overflow-y-auto rounded-lg border border-border">
                    <li v-for="medication in results" :key="medication.id">
                        ...
                        >
                            <span class="min-w-0">
                                <span class="block truncate font-bold text-foreground">{{ medication.name }}</span>
                                <span class="block truncate text-xs text-muted-foreground">
                                    {{ medication.type }} · {{ medication.dosage }} · {{ $t('enums.dose_form.' + medication.dose_form) }}
                                </span>
                            </span>
                            <span class="shrink-0 text-xs text-muted-foreground">{{ medication.ndc }}</span>
                        </button>
                    </li>
                </ul>
            </div>
